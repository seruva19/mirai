"""Collect lineage-bound REAP/MAN/MSAN/frequency pruning evidence."""
...
from mirai.core.training.calibration.expert_pruning import (
...
def calibrate_expert_pruning(
    ...
    *,
    ...
    """Build an isolated session and collect the configured pruning criterion."""
    normalize_calibrated_expert_pruning_criterion(
    ...
    session = None
    with tempfile.TemporaryDirectory(prefix="mirai-expert-pruning-") as temp_dir:
        ...
        try:
            with acquire_gpu_lease(
                ...
                report = run_expert_pruning_calibration_session(
                    session,
                    output_path=output,
                    calibration_steps=int(calibration_steps),
                    criterion=str(config.model.params.expert_pruning_criterion),
...
def main() -> int:
    ...
    config, _notes = load_runtime_config(
        args.config,
        entrypoint="expert-pruning-calibration",
    )
    summary = calibrate_expert_pruning(
        config,
