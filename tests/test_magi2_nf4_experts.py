...
_requires_nf4 = pytest.mark.skipif(
    not _NF4, reason="NF4 storage requires CUDA and bitsandbytes"
)
...
def _sorted_layout(
    *, groups: int, rows_per_group: list[int], columns: int, dtype: torch.dtype
):
    offsets = torch.tensor(
        list(torch.tensor(rows_per_group).cumsum(0).tolist()), dtype=torch.int32
    )
    total = int(offsets[-1])
...
def test_dequantized_segment_layout_satisfies_the_grouped_precondition() -> None:
    ...
    assert (
        grouped_mm_stride_violations(
            (grouped_mm_operand(segment, label="dequantized segment"),)
        )
        == ()
    )
...
def test_quantized_storage_adds_exactly_three_consumed_policy_fields() -> None:
...
def test_quantized_policy_rejection_covers_every_unconsumed_field() -> None:
    ...
    assert (
        set(_QUANTIZED_NON_DEFAULT_POLICY_VALUES)
        | set(_QUANTIZED_CONSUMED_POLICY_FIELDS)
        == declared
    )
...
def test_packed_experts_leave_no_reference_loop_to_fall_back_to() -> None:
    ...
    plan = resolve_magi2_moe_execution(
        MoEOptimizationPolicy(), quantized_experts=True
    )
    assert plan == Magi2GroupedMoEPlan(forward_backend="auto", dx_backend="auto")
...
def test_installing_stores_removes_the_dense_expert_state() -> None:
    ...
    assert install_magi2_nf4_expert_stores(container)["mlp.moe_mlp"] is stores[
        "mlp.moe_mlp"
    ]
...
def _run(module: torch.nn.Module, adapter: LowRankWeight, hidden_size: int):
    torch.manual_seed(7)
    hidden = torch.randn(
        16, hidden_size, device="cuda", dtype=torch.bfloat16, requires_grad=True
    )
...
def test_nf4_grouped_execution_matches_the_reference_loop_on_its_own_weights(
    ...
    """
    hidden_size = 256
    packed = _reduced_moe(device=torch.device("cuda"), dtype=torch.bfloat16,
                          hidden_size=hidden_size)
    reference = _reduced_moe(device=torch.device("cuda"), dtype=torch.bfloat16,
                             hidden_size=hidden_size)
    stores = quantize_magi2_experts_in_place(_container(packed))
    store = stores["mlp.moe_mlp"]
    store.set_expert_weight_access_policy(
        expert_weight_access=access, expert_dequant_chunk_size=3
    )
    with torch.no_grad():
...
def test_nf4_grouped_execution_stays_within_quantization_error_of_bf16() -> None:
    ...
    hidden_size = 256
    bf16 = _reduced_moe(device=torch.device("cuda"), dtype=torch.bfloat16,
                        hidden_size=hidden_size)
    packed = _reduced_moe(device=torch.device("cuda"), dtype=torch.bfloat16,
                          hidden_size=hidden_size)
...
def test_packed_experts_never_enter_autograd() -> None:
    ...
    assert not any(
        name in module._parameters for name in MAGI2_ROUTED_EXPERT_TENSOR_NAMES
    )
    # The packed payload is a fraction of the dense stack it replaced.
    dense_bytes = sum(
        int(torch.tensor(shape).prod().item()) * 2
        for shape in (
            store.expert_weight_shape(key)
            for key in MAGI2_ROUTED_EXPERT_TENSOR_NAMES
        )
    )
...
def test_streaming_load_names_a_missing_routed_tensor(tmp_path) -> None:
    ...
    with pytest.raises(Magi2QuantizedExpertError, match="mlp.moe_mlp.W_up"):
        stream_quantize_magi2_experts(
            _container(module), checkpoint_dir=str(tmp_path)
        )
...
def test_streaming_load_packs_every_routed_tensor_from_its_shard(tmp_path) -> None:
    ...
    dense = {
        name: getattr(source, name).detach().clone()
        for name in MAGI2_ROUTED_EXPERT_TENSOR_NAMES
    }
    ...
    assert not any(
        name in target._parameters for name in MAGI2_ROUTED_EXPERT_TENSOR_NAMES
    )
    for name in MAGI2_ROUTED_EXPERT_TENSOR_NAMES:
